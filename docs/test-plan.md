...
| Workspace Inspector | Collapsible/resizable behavior, logically grouped X-Hover Overall/Tabletop/End boxes/Support layout/Top support members/Bottom support members/Support joinery/Routing templates controls, semantic top/bottom support selectors, weighted-center controls only for paper towel holder, compact-bracket envelope/strength/two-up groups, pipe-clamp-bed surface/fit/snap groups and fit-coupon link, miter-runner measured-slot/clearance controls with a derived finished width, router-mortise and router-tenon preset markers plus M5 hardware summaries, bandsaw-sled wood/print/hardware cut list, X-Hover Assembled/Exploded/Cut List/Templates modes, rendering modes, original overlay, and audit rows. |
...
| Parameter Limits | Static limits plus dependent holder tube/diameter limits, tray floor/height limits, compact-bracket source-section minimums and two-up plate envelope, metric-nut-knob bore/pocket/guard/handle wall and roof constraints, pipe-clamp-bed length/clip bridge, pipe/bed shoulder, crown-cap, clip-wall, and easy-release undercut constraints, miter-runner slot/clearance coupling that never undercuts the fixed 18.8 mm Main Part, and Hover-table widened overhang/spread/support ranges, independent top/bottom box radii, six support-layout combinations, radius-aware placement, round-overs, conditional half-laps, and direct-contact relationships. |
| Structural Screening | X-Hover and Plate Table overall and six component grades stay finite and bounded. Height, post/end-box dimensions, plate geometry, support topology, independent Plate foot extensions, and C-channel section/coverage/distribution edits move the appropriate racking, joint, torsion, tipping, rocking, and member/tabletop-stiffness scores monotonically. The router-tenon jig separately screens its base strip and active-guide cantilever under a 75 N comparison load. The bandsaw sled screens its two gusseted brackets under a 100 N lateral comparison load, verifies anti-yaw spacing, derives longer slots through the 8 in bracket range while retaining 12 mm end webs, keeps gusset length independent, and keeps all calculations explicitly non-certifying. The UI exposes formula inputs, ±1 in overall-height sensitivity, and required physical-test caveats. |
...
| Export | Download starts, file name includes model prefix and parameter keys, generated STL is non-empty, compact-bracket single and two-up exports retain manifold topology, source section minimums, Z = 0, current gap, and plate bounds, metric-nut-knob export retains one manifold shell with the current handle, guard, bore, and pocket dimensions, pipe-clamp-bed export retains one manifold shell in support-face-down orientation while excluding the preview pipe and the separate fit coupon remains downloadable, miter-runner export retains the selected finished width with invariant source angles and Z = 0, router-mortise export yields ten individual current-parameter files, router-tenon export yields five individual current-parameter files, bandsaw-sled export yields four individual printed-part files named with bracket and gusset lengths while keeping wood parts out of STL output, furniture-model exports retain finite nondegenerate triangles plus their scaled envelope, and each full-size routing-template STL fits the selected square plate with the requested thickness and keyed seam. |
