...
| Structural Screening | X-Hover overall and six component grades stay finite and bounded; height, end-box dimensions, and support-layout edits move the appropriate racking, torsion, tipping, rocking, and member-stiffness scores monotonically. The UI exposes ±1 in overall-height sensitivity and the required physical-test caveat. |
| Viewer | Nonblank canvas after load, parameter or support-layout edit, X-Hover 11–13-piece explosion, variant-aware cut sheet, and segmented routing-template preview, render mode, original overlay, zoom, cube orientation, reset, center view, and sidebar collapse. Assembly switching and live exploded/cut-list/template edits preserve camera state. |
