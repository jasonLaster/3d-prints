...
| Workspace Inspector | Collapsible/resizable behavior, logically grouped X-Hover Overall/Tabletop/End boxes/Support layout/Top support members/Bottom support members/Support joinery/Routing templates controls, semantic top/bottom support selectors, weighted-center controls only for paper towel holder, router-mortise and router-tenon preset markers plus M5 hardware summaries, bandsaw-sled wood/print/hardware cut list, X-Hover Assembled/Exploded/Cut List/Templates modes, rendering modes, original overlay, and audit rows. |
...
| Parameter Limits | Static limits plus dependent holder tube/diameter limits, tray floor/height limits, desk-top band/profile/strip limits, and Hover-table widened overhang/spread/support ranges, independent top/bottom box radii, six support-layout combinations, radius-aware placement, round-overs, conditional half-laps, and direct-contact relationships. |
| Structural Screening | X-Hover and Plate Table overall and six component grades stay finite and bounded. Height, post/end-box dimensions, plate geometry, support topology, independent Plate foot extensions, and C-channel section/coverage/distribution edits move the appropriate racking, joint, torsion, tipping, rocking, and member/tabletop-stiffness scores monotonically. The router-tenon jig separately screens its base strip and active-guide cantilever under a 75 N comparison load. The bandsaw sled screens its two gusseted brackets under a 100 N lateral comparison load, verifies anti-yaw spacing, and keeps all calculations explicitly non-certifying. The UI exposes formula inputs, ±1 in overall-height sensitivity, and required physical-test caveats. |
