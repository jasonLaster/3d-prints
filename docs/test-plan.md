...
| Structural Screening | X-Hover overall and six component grades stay finite and bounded; height, end-box dimensions, support-layout edits, and C-channel section/coverage/distribution edits move the appropriate racking, torsion, tipping, rocking, and member/tabletop-stiffness scores monotonically. The UI exposes every channel-derived input, ±1 in overall-height sensitivity, and the required physical-test caveat. |
