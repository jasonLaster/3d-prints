...
| Structural Screening | X-Hover and Plate Table overall and six component grades stay finite and bounded. Height, post/end-box dimensions, plate geometry, support topology, independent Plate foot extensions, and C-channel section/coverage/distribution edits move the appropriate racking, joint, torsion, tipping, rocking, and member/tabletop-stiffness scores monotonically. The router-tenon jig separately screens its base strip and active-guide cantilever under a 75 N comparison load. The bandsaw sled screens its two gusseted brackets under a 100 N lateral comparison load, verifies anti-yaw spacing, supports bracket lengths through 8 in with an automatically proportional half-length gusset, and keeps all calculations explicitly non-certifying. The UI exposes formula inputs, ±1 in overall-height sensitivity, and required physical-test caveats. |
