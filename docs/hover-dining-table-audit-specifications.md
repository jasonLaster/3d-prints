...
Bottom spread is the only splay hypothesis. Zero spread produces orthogonal side members. A nonzero value changes the bottom outer and inner widths symmetrically. The upper endpoint derives from the top opening and the lower endpoint derives independently from the bottom opening, so the lower X follows a spread edit without entering the corner curve or moving any floor contact below `Z = 0`.
...
Upper and lower brace width and vertical thickness remain independently editable so the floor X can be more architectural than the largely concealed upper X. Their edge-radius controls create top-and-bottom long-edge round-overs, not rounded plan ends. The round-overs stop at flat end-box-aligned cuts and must never consume a brace cross-section or round away a half-lap shoulder.
...
Exploded mode is a presentation of 13 independently movable pieces before glue-up: one tabletop, two end boxes with two horizontal rails and two vertical stiles each, and two X assemblies with two bars each. The tabletop may remain one piece. Each X member retains the same mitered ends, top-and-bottom round-overs, and one half of its centered half-lap used by the assembled geometry.

The four separated bars at each end show the rail-and-stile stock that produces the finished closed box. The routed inner and outer corner curves and face-edge round-overs in the assembled model remain the final-shape source of truth after glue-up; exploded mode must not imply four permanently square corners.

Exploded offsets are presentation-only. Switching between Assembled and Exploded must not change any parameter, URL value, assembled STL, oak material, render mode, camera orientation, or zoom. A parameter edit made in Exploded mode must regenerate and reposition all 13 pieces while retaining that mode and the current camera.

## Dimensioned cut-list contract

Cut List is a third assembly view, alongside Assembled and Exploded. It combines an eight-line grouped schedule with true-shape SVG views for the 13 physical pieces: T1 tabletop (qty 1), B1 top rails (qty 2), B2 bottom rails (qty 2), B3 mirrored stiles (qty 4), and separate A/B members for the upper and floor Xs (qty 1 each). Separating each X member is required because one lap is cut from the top face and its mate from the bottom.

Every row and drawing is associative to the full-size furniture parameters, not the manipulation-model scale. Each part shows finished nominal length, width, thickness, quantity, lengthwise oak grain, and any applicable processing dimensions. Brace drawings additionally show the true member length between the parallel end-box contact planes, end-miter angle from square, centered half-lap location, lap width, lap depth, fit clearance, and lap face. Stile drawings show their true sloped length and end-cut angle when bottom spread is nonzero.

The sheet deliberately distinguishes finished nominal dimensions from rough stock. It tells the builder to add their own rough-milling allowance rather than silently increasing dimensions because allowance depends on stock condition and shop process. The B1–B3 drawings represent pre-glue-up blanks; the editable outer/inner corner radii and face-edge round-over remain post-glue-up routing operations.
...
- inward endpoint migration after increasing the editable inner corner radius, with the complete angled end remaining tangent-clear;
...
- exactly 13 exploded pieces comprising one top, four horizontal box rails, four vertical box stiles, two upper-X bars, and two floor-X bars;
- an Assembled/Exploded switch that preserves parameter state, render mode, export geometry, camera orientation, and zoom; and
- eight cut-list schedule lines whose quantities sum to 13, with true-shape SVG views, full-size finished dimensions, grain arrows, and complete half-lap dimensions;
