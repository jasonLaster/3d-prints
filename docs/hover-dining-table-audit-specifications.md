...
Top and bottom support layouts each expose their own member width, vertical thickness, bearing-zone inset, and bottom-edge radius. Both bearing-zone inset controls share the same configured 0–12 in range and then receive the same geometry-derived tangent-clearance clamp. When both layouts are X crossbars, an opt-in synchronization control may match all four member dimensions. Enabling synchronization uses the current top width, thickness, inset, and edge radius as the source, clamps each pair to its shared safe range, and applies the result to the bottom X. Subsequent edits from either top or bottom control update the corresponding partner; the setting remains dormant while either layout is not an X and reconciles from the top values when both become Xs again. X lateral endpoints remain independently derived from the corresponding inner-corner tangencies; upper-stretcher lateral placement derives from the top rail's outside bearing edge; and the floor board remains centered. Each edge-radius control rounds only the bottom long edge, leaving the top edge square; plan ends remain square. X crossbar radii stop short of the half-lap shoulder. Straight-support radii have no half-lap restriction and may instead grow to the smaller of half the member width and nearly its full thickness, permitting a near-quarter-round bottom profile while preserving a flat top face. Switching a straight support back to an X clamps its radius to the X-safe range. Half-lap fit clearance applies only while at least one X is selected.
