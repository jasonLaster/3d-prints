...
| Triangular Retaining Nut Knob provides a continuous palm-friendly grip and a tunable press-on bolt detent | `docs/triangular-retaining-knob-audit-specifications.md`; Triangular Retaining Nut Knob model JSON | Model audit checks triangle defaults, independent handle/guard/fastener/retention controls, collar placement, roof, manifold topology, tip span, height, and Z = 0; focused Playwright checks default and large geometry, collar cross-section, zero-interference fallback, URL persistence, audit UI, responsive layout, and STL export |
