...
| Handheld Router Tenon Jig derives external guide openings from the tenon, cutter, and bearing | `docs/router-tenon-jig-audit-specifications.md`; router-tenon-jig model JSON | Source audit checks external bearing-guide math, six blind M5 heat-set pockets, thickness/width witness marks, shoulders, and all five manifold STL files; focused Playwright checks presets, coupled limits, preview-only router and depth-stop status, responsive rendering, URL state, and five individual downloads |
