...
| Compact wall bracket fits two copies without shrinking structural sections | `docs/compact-wall-bracket-audit-specifications.md`; Compact Wall Bracket model JSON | Source audit checks the retained source SHA-256 and 190.9188 × 25.6 × 99.9285 mm bounds, confirms no bolt bores, preserves the 25.6 mm depth, 10 mm base, and 6.4 mm diagonal/web minimums, and audits the single and 235 × 60.2 mm two-up STL bytes; focused Playwright checks grouped controls, dependent plate limits, URL state, audit rows, canvas rendering, and both exports |
...
| Parametric Metric Nut Knob reconstructs the supplied M8 mesh without uniform scaling | `docs/metric-nut-knob-audit-specifications.md`; Parametric Metric Nut Knob model JSON | Source audit checks byte custody, measured defaults, the recorded source topology defect, and the replacement STL's envelope/topology; focused Playwright covers independent handle, lobe, spacer / guard, bolt, nut-pocket, URL, canvas, audit, and export behavior across default and extreme states |
| Pipe Clamp Bed supports a 1-inch board on standard 3/4-inch pipe clamps | `docs/pipe-clamp-bed-audit-specifications.md`; Pipe Clamp Bed model JSON | Source audit checks byte custody, documented unit interpretation, 26.67 mm pipe fit, 1.6 mm crown lift, 27.635 mm workpiece center, easy-release deflection, multiple parameter states, full-bed and fit-coupon topology, Z = 0, and plate fit; focused Playwright covers grouped controls, pipe preview, URL persistence, audit rows, coupon access, mobile layout, and current-parameter STL export |
...
| Adjustable Fence Bandsaw Sled separates wood fabrication from printed locking hardware | `docs/bandsaw-sled-audit-specifications.md`; bandsaw-sled model JSON | Source audit checks the wood cut plan, 8 in bracket range, derived 7.06 in maximum slot, preserved 12 mm slot-end webs, independent shorter gussets, supporting base depth, two-bracket anti-yaw spacing, continuous slot travel, M5 heat-set and M6 wood-insert stack-ups, bracket strength/deflection screens, and all four manifold STL files; focused Playwright checks the derived 8 in / 7+ in slot extreme, restored gusset control, removed redundant slot control, four-material legend, live fence travel, reloadable URL state, audit rows, and four individual downloads |
