...
| X-Hover table preserves the supplied tabletop and end-box silhouette while replacing the base connection with two horizontal X assemblies | Hover table model JSON and `docs/hover-dining-table-audit-specifications.md` | Static audit proves four radius-aware flush end miters, top/bottom brace round-overs, two centered 50/50 half-laps, zero parallel stretchers or pads, upper contact at the tabletop underside, lower contact at the floor, and proportional regeneration; Playwright changes the inner radius, verifies inward endpoint migration, manipulates the X controls, and parses finite nondegenerate export geometry. |
| X-Hover table has a pre-glue-up exploded mode | Hover table model JSON and `docs/hover-dining-table-audit-specifications.md` | Geometry tests require exactly 13 finite nondegenerate named pieces with one top, eight end-box bars, and four half-lapped X bars; Playwright switches Assembled/Exploded, edits the inner radius while exploded, and verifies camera preservation plus unchanged assembled export. |
| X-Hover table has a dimensioned fabrication sheet | Hover table model JSON and `docs/hover-dining-table-audit-specifications.md` | Cut-list tests require eight grouped schedule lines totaling 13 pieces, full-size finished dimensions invariant under mock-scale edits, true-shape SVGs, grain direction, and complete half-lap dimensions; Playwright verifies Cut List mode and exports the assembled envelope while the 2D sheet is active. |
...
- Rendering, X-Hover assembly switching, weighted-center, original-overlay, actions menu, viewer camera controls, cube orientation, or zoom controls throw page errors or leave the canvas blank.
