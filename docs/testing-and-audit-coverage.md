...
| Compact wall bracket preserves the source proportion, eight drill holes, and an opposed rotated pair with distinct body and brace depths | `docs/compact-wall-bracket-audit-specifications.md`; Compact Wall Bracket model JSON | Source audit checks the retained SHA-256, 190.9188 × 25.6 × 99.9285 mm bounds, four major depth planes, 8.96 mm recessed core, and eight measured 4 mm through-holes; it verifies the derived 104.6817 mm rise, 19.05/12.7 mm stepped depths, hole edge/web limits, planar section minimums, and one-shell plus 232.37 mm square two-shell STL bytes. Focused Playwright checks thirteen grouped controls, dependent depth/hole/plate limits, URL state, audit rows, canvas rendering, and both exports. |
