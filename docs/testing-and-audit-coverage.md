...
| Compact wall bracket preserves the source proportion and fits an opposed rotated pair with distinct body and brace depths | `docs/compact-wall-bracket-audit-specifications.md`; Compact Wall Bracket model JSON | Source audit checks the retained SHA-256, 190.9188 × 25.6 × 99.9285 mm bounds, four major depth planes, 8.96 mm recessed core, and absent bolt bores; it verifies the derived 104.6817 mm rise, 19.05/12.7 mm stepped depths, planar section minimums, and one-shell plus 232.37 mm square two-shell STL bytes. Focused Playwright checks nine grouped controls, dependent depth/plate limits, URL state, audit rows, canvas rendering, and both exports. |
