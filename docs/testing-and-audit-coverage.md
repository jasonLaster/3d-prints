...
| Adjustable Fence Bandsaw Sled separates wood fabrication from printed locking hardware | `docs/bandsaw-sled-audit-specifications.md`; bandsaw-sled model JSON | Source audit checks the wood cut plan, two-bracket anti-yaw spacing, continuous slot travel, M5 heat-set and M6 wood-insert stack-ups, bracket strength/deflection screens, and all four manifold STL files; focused Playwright checks the four-material legend, live fence travel, reloadable URL state, audit rows, and four individual downloads |
| White Oak Desk Top keeps surface choices and routed edges fabrication-consistent | `docs/desk-tabletop-audit-specifications.md`; desk-tabletop model JSON | Source audit checks the 1/8 in veneer default, 1–2 in solid band, profile stock, flooring range, and reference STL; focused geometry/browser coverage checks the scaled envelope, continuous versus staggered surface geometry, dependent limits, conditional strip controls, URL state, audits, and single-model export |
