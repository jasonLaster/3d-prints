...
- Playwright tests validate the running React app, including the 3D canvas, controls, URL state, unit conversion, model switching, themes, and sidebar resizing.
...
| Pan, zoom, frame, and edge-oriented views are easy to use in 3D | This coverage document | Playwright clicks zoom, pan, frame, top, X edge, and Y edge controls and verifies the canvas remains nonblank with no page errors |
...
| App supports multiple STLs with per-model JSON for parameters, audit, and scripts | README model structure; each model `model.json` | Model audit runner discovers catalog entries and executes each declared model audit script; Playwright switches between catalog models |
...
| Convex library stores saved versions, forks, and uploaded STL assets | `docs/convex-library.md`; Convex schema and functions | Static Playwright checks require Convex schema/functions/docs; app tests verify the Library section renders |
...
- Rendering, weighted-center, original-overlay, camera, pan, or zoom controls throw page errors or leave the canvas blank.
- The sidebar rail cannot be resized by pointer and keyboard.
- Convex library schema, functions, and documentation are missing or drift from the Save/Fork/Upload/Open flows.
