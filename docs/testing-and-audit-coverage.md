...
| Variable pipe wall mount creates one fitted hook per entered pipe and reports usable drill centers | `docs/pipe-wall-mount-audit-specifications.md`; Variable Pipe Wall Mount model JSON | Source audit preserves the supplied hook SHA-256 and measured defects, verifies the default three one-inch cradles and a four-diameter mixed set, checks total/per-side wiggle room, height/reach coupling, four bore walls, preview diameter parity, and single-shell broad-side-down STL bytes; focused Playwright checks the comma-delimited pipe list, grouped controls, reloadable compact URL, drill/audit text, translucent preview updates, reference overlay, responsive canvas, and export topology/bounds |
