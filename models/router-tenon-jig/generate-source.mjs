...
const configPath = path.join(root, "public/models/router-tenon-jig/model.json");
const bundlePath = path.join(os.tmpdir(), `router-tenon-jig-${process.pid}-${Date.now()}.mjs`);
...
try {
  await build({
    bundle: true,
    entryPoints: [path.join(root, "src/models/routerTenonJig.ts")],
    format: "esm",
    outfile: bundlePath,
    platform: "node",
  });
  const { createRouterTenonJigPartGeometries } = await import(
  ...
  for (const part of parts) {
    ...
    const outputPath = path.join(root, "public/models/router-tenon-jig", definition.fileName);
    fs.writeFileSync(
      outputPath,
