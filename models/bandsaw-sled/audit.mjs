...
for (const key of [
  ...
  "bracketWidth", "bracketDepth", "bracketGussetDepth", "lockBoltDiameter",
...
const slotLength = bracketDepth - model.geometry.minimumSlotEndWeb * 2;
...
assert(Math.abs(slotLength - 66) <= 1e-6, "default slot is derived as 66 mm");
...
const longSlotLength = longBracketDepth - model.geometry.minimumSlotEndWeb * 2;
assert(Math.abs(longSlotLength - 179.2) <= 1e-6, "8 inch bracket derives a 179.2 mm slot");
assert(longSlotLength >= 177.8, "8 inch bracket derives at least a 7 inch slot");
assert(Math.abs((longBracketDepth - longSlotLength) / 2 - model.geometry.minimumSlotEndWeb) <= 1e-6, "maximum slot preserves both 12 mm end webs");
...
for (const part of model.parts) {
  ...
  if (part.key.includes("bracket")) {
    ...
    assert(info.triangles > 900, `${part.label} includes a derived long slot, independent gussets, and two stepped insert pockets`);
  } else {
