...
const configPath = path.join(root, "public/models/bandsaw-sled/model.json");
const bundlePath = path.join(
  os.tmpdir(),
  `bandsaw-sled-${process.pid}-${Date.now()}.mjs`,
);
...
try {
  await build({
    bundle: true,
    entryPoints: [path.join(root, "src/models/bandsawSled.ts")],
    format: "esm",
    outfile: bundlePath,
    platform: "node",
  });
  const { createBandsawSledPartGeometries } = await import(
